...
from neptunesdr_firmwave.manifest import finish_runtime_manifest  # noqa: E402
from neptunesdr_firmwave.runtime_rootfs import (  # noqa: E402
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        published = [
            ("kernel", candidate.artifacts.kernel, "P210 Linux kernel payload"),
            ("devicetree", candidate.artifacts.devicetree, "P210 flattened device tree"),
            ("rootfs", candidate.artifacts.ramdisk, "ADI PlutoSDR v0.39 initramfs"),
        ]
        ...
        if args.iiod_exec_probe:
            ...
            published.append(
                ("iiod_exec_probe", path, "test-only released iiod execution probe initramfs")
            )
        if args.tcp_iiod:
            ...
            published.append(
                ("tcp_iiod", path, "test-only network iiod service initramfs")
            )
        if args.tcp_probe_init:
            ...
            published.append(
                ("tcp_probe_init", path, "test-only released iiod PID-1 probe initramfs")
            )
        if args.fft_streamer:
            ...
            published.append(
                ("fft_runtime", path, "QEMU P210 IIO-DMAC/PL-FFT/NSFT development initramfs")
            )
